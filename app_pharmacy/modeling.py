...
def select_report_model_name(results):
    """Возвращает модель, которая должна использоваться в отчётных артефактах."""
    if not isinstance(results, dict):
        raise ValueError("results должен быть словарём")

    explicit_name = results.get('_report_model_name')
    if explicit_name in results and isinstance(results.get(explicit_name), dict):
        return explicit_name

    candidates = [
        (name, payload)
        for name, payload in results.items()
        if not name.startswith('_') and isinstance(payload, dict)
    ]
    if not candidates:
        raise ValueError("В results нет моделей для формирования отчёта")

    return max(candidates, key=lambda item: item[1].get('f1', float('-inf')))[0]
...
def train_models(X, y, h3_grid=None, use_spatial_cv=None, n_clusters=None):
    ...
    results['_report_model_name'] = best_name
