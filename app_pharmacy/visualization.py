...
setTimeout(function() {{
  try {{
    ...
    if (map && map.attributionControl) {{
      map.attributionControl.setPrefix("PharmaLoc");
    }}
  }} catch (e) {{}}
}}, 0);
"""))
...
def plot_feature_importance(model, feature_names, filename='feature_importance.png'):
    ...
    def _extract_feature_estimator(obj):
        if obj is None:
            return None
        ...
        if hasattr(obj, 'named_steps'):
            classifier = obj.named_steps.get('classifier')
            estimator = _extract_feature_estimator(classifier)
            if estimator is not None:
                return estimator
            for step in obj.named_steps.values():
                estimator = _extract_feature_estimator(step)
                if estimator is not None:
                    return estimator

        if hasattr(obj, 'calibrated_classifiers_'):
            for calibrated in obj.calibrated_classifiers_:
                for attr in ('estimator', 'estimator_', 'base_estimator', 'base_estimator_'):
                    estimator = _extract_feature_estimator(getattr(calibrated, attr, None))
                    if estimator is not None:
                        return estimator

        for attr in ('estimator', 'estimator_', 'base_estimator', 'base_estimator_'):
            estimator = _extract_feature_estimator(getattr(obj, attr, None))
            if estimator is not None:
                return estimator

        return None

    clf = _extract_feature_estimator(model)
    if clf is None:
        print("⚠️ Модель не поддерживает feature importance")
        return
    ...
    if len(importances) != len(feature_names):
        print("⚠️ Размерность feature importance не совпадает с feature_names")
...
def plot_confusion_matrix(y_true, y_pred, model_name='Model', filename='confusion_matrix.png'):
    ...
    axes[0].set_title(f'{display_name}: Абсолютные значения', fontsize=12, fontweight='bold')
    ...
    axes[1].set_title(f'{display_name}: Нормализованные (по классам)', fontsize=12, fontweight='bold')
...
def plot_cluster_profiles(df, cluster_col='cluster', features=None, filename='cluster_profiles.png'):
    ...
    cluster_effects = ((cluster_means - overall_mean) / overall_std).fillna(0.0)
    ranked_features = cluster_effects.abs().max(axis=0).sort_values(ascending=False)
    selected_features = ranked_features.index[:min(10, len(ranked_features))]
    cluster_effects = cluster_effects[selected_features]

    n_clusters = len(cluster_effects)
    fig_height = max(6, len(selected_features) * 0.7)
    fig, ax = plt.subplots(figsize=(14, fig_height))

    y = np.arange(len(selected_features))
    bar_height = 0.8 / max(n_clusters, 1)
    colors = ['#d73027', '#4575b4', '#1a9850', '#984ea3']

    for i, (cluster, row) in enumerate(cluster_effects.iterrows()):
        offset = (i - (n_clusters - 1) / 2) * bar_height
        ax.barh(
            y + offset,
            row.values,
            bar_height,
    ...
    ax.set_xlabel('Отклонение от общего среднего (σ)', fontsize=12)
    ax.set_title('Профили кластеров (отклонение средних по признакам)', fontsize=14, fontweight='bold', pad=20)
    ax.set_yticks(y)
    ax.set_yticklabels([f.replace('_', '\n') for f in selected_features], fontsize=10)
    ax.invert_yaxis()
...
def plot_business_metrics(results, model_name='Best Model', filename='business_metrics.png'):
    ...
    plt.suptitle(f'Бизнес-метрики модели: {display_name}', fontsize=16, fontweight='bold')
