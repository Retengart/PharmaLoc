...
UV_RUN_CMD = "uv run --with-requirements app_pharmacy/requirements.txt python -m app_pharmacy.main"
...
Примеры использования:
  uv run --with-requirements app_pharmacy/requirements.txt python -m app_pharmacy.main
  uv run --with-requirements app_pharmacy/requirements.txt python -m app_pharmacy.main --train
  uv run --with-requirements app_pharmacy/requirements.txt python -m app_pharmacy.main --load
  uv run --with-requirements app_pharmacy/requirements.txt python -m app_pharmacy.main --skip-data
  ...
  uv run --with-requirements app_pharmacy/requirements.txt python -m app_pharmacy.main --enrich
        """
    )
...
def run_validation_mode():
    ...
    if not model_exists:
        ...
        print(f"   {UV_RUN_CMD} --train --no-leakage")
        return
...
def main():
    ...
    if need_features:
        ...
        if leakage_features_present and not exclude_leakage:
            ...
            print(f"   Пример: {UV_RUN_CMD} --train --no-leakage")
    ...
    if h3_grid['has_pharmacy'].sum() > 5:
        ...
        if results is not None:
            ...
            best_model_name = modeling.select_report_model_name(results)
            print("\n📊 Визуализация бизнес-метрик...")
